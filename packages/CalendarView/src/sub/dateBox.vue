...
export default {
    ...
    methods: {
        ...
        async daysInit () {
            let d = [];
            let nowday = this.$SDate.Parse(this.$SDate.DateToString(this.thisValue));
            nowday.setDate(1);
            ...
            for(let i = 0; i < 2; i++) {
                for(let j = 0; j < this.dayList[this.leapYear(nowday.getFullYear())][nowday.getMonth()]; j++) {
                    d.push({ year: nowday.getFullYear(), month: nowday.getMonth(), no: j + 1});
                }
                nowday.setMonth(nowday.getMonth() + 1);
            }
            this.days = d;
            await this.delay(50);
            this.$refs.main.scrollTop = this.$refs.main.scrollTop + this.size * 2;
            this.loadPrev();
        },
        ...
        async loadPrev () {
            ...
            await this.delay(30);
            this.$refs.main.scrollTop = this.$refs.main.scrollTop + count * this.size;
        },
        async loadNext () {
            let nowday = new Date();
            nowday.setDate(1);
            ...
            for(let j = 0; j < this.dayList[this.leapYear(nowday.getFullYear())][nowday.getMonth()]; j++) {
                this.days.push({ year: nowday.getFullYear(), month: nowday.getMonth(), no: j + 1});
            }
            await this.delay(30);
        },
        async slide (val) {
            ...
            let val_date = this.$SDate.Parse(`${val.year}-${val.month + 1}-1 0:0:0`);
            let current_date = this.$SDate.Parse(`${this.currentRange.year}-${this.currentRange.month + 1}-${this.currentRange.no} 0:0:0`);
            clearInterval(this.timer.scroller);
            return await new Promise(resolve => {
                this.timer.scroller = setInterval(() => {
                    let target = this.days.find(
                        item => item.year == val.year && item.month == val.month && item.no == 1
                    );
                    if(target == undefined) {
                    ...
                    let speed = -Math.floor((this.$refs.main.scrollTop - height) / 2);
