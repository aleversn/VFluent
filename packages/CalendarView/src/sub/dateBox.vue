  ...
  methods: {
    ...
    async daysInit() {
      ...
      for (
        ...
        j <
        this.dayList[this.leapYear(lastday.getFullYear())][lastday.getMonth()];
        j++
      ) {
        d.push({
      ...
      for (let i = 0; i < 2; i++) {
        for (
          let j = 0;
          j <
          this.dayList[this.leapYear(nowday.getFullYear())][nowday.getMonth()];
          j++
        ) {
          d.push({
    ...
    async loadNext() {
      ...
      if (nowday.getFullYear() == this.end && nowday.getMonth() == 11) return 0;
      ...
      return await new Promise(resolve => {
        this.timer.scroller = setInterval(() => {
          let target = this.days.find(
            item =>
              item.year == val.year && item.month == val.month && item.no == 1
          );
          if (target == undefined) {
