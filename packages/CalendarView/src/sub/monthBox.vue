...
<div class="picker-container" ref="main">
    <button v-for="(item, index) in months" :key="`year: ${index}`" class="btn" :class="{range: item.year === currentRange.year, choose: item.year === nowYear && item.no === nowMonth}" @click="choose(item)">{{item.name}}</button>
</div>
</template>
