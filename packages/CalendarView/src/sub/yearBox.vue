...
<div class="picker-container" ref="main">
    <button v-for="(item, index) in years" :key="`year: ${index}`" class="btn" :class="{range: item >= currentRange && item - currentRange < 10, choose: item == nowYear}" @click="choose(item)">{{item}}</button>
</div>
</template>
