<template>
    <div
        name="fv-DatePicker"
        :class="['fv-' + $theme + '-DatePicker', { disabled: disabled }]"
    >
        <!-- Outside Box -->
        <div
        ...
        >
            <button
                v-if="!hideMonth"
                class="fv-DatePicker__input-month"
            >{{ months[value.getMonth()] }}</button>
            <button
                v-if="!hideDay"
                class="fv-DatePicker__input-day"
            ...
            <button
                v-if="!hideYear"
                :style="{ borderColor: innerBorderColor }"
                class="fv-DatePicker__input-year"
            >{{
                    value.getFullYear()
            }}</button>
        </div>
        ...
        <transition name="fv-DatePicker__options">
            ...
            >
                <div class="fv-DatePicker__input-body">
                    ...
                    <div
                        v-if="!hideMonth"
                        class="fv-DatePicker__input-options-col"
...
export default {
    ...
    methods: {
        ...
        setDayOptions() {
            ...
            this.$nextTick(() => {
                let origin = (this.$refs.day.scrollTop =
                    40 * (this.config.buffer - 4));
                ...
                this.optionsConfig.day.scroll = () => {
                    ...
                    this.slideCol(
                        ...
                        () => {
                            ...
                            let day = this.selected.date.getDate();
                            this.selected.date.setDate(
                                this.nNext(day, dayRange, 1)
                            );
                            this.options.day.push(
                                this.nNext(
                                    day,
                                    dayRange,
                                    this.config.buffer + 1
                                )
                            );
                        },
                        () => {
            ...
            this.$nextTick(() => {
                ...
                this.optionsConfig.year.scroll = async () => {
                    ...
                    if (this.$refs.year.scrollTop - origin > 0) {
                        if (
                            this.adjustYear(
                                this.selected.date.getFullYear() + 1
                            ) < 0
                        ) {
