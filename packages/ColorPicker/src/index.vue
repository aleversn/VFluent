<template>
    <div :class="'fv-' + $theme + '-colorPicker'">
        <div
            ref="chooseBox"
            class="color-picker__choose_box"
            @mousedown="
                onclick($event, clickBox) &&
                    moveable($event, 'chooseBox', syncBoxButton)
            "
            @touchstart="moveable($event, 'chooseBox', syncBoxButton)"
            :style="chooseStyle"
        >
            <!-- Light Mask -->
        ...
        <div class="color-picker__slider_box">
            <div class="slider">
                <div
                    ref="hueBox"
                    @mousedown="
                        onclick($event, clickHue) &&
                            moveable($event, 'hueBox', syncHueButton)
                    "
                    @touchstart="moveable($event, 'hueBox', syncHueButton)"
                    class="hue_box"
                >
                    <div
                        class="button"
                        ref="hueButton"
                        :style="hueButtonStyle"
                        @mousedown="moveable($event, 'hueBox', syncHueButton)"
                        @touchstart="moveable($event, 'hueBox', syncHueButton)"
                    ></div>
                </div>
                <div
                    ref="alphaBox"
                    @mousedown="
                        onclick($event, clickAlpha) &&
                            moveable($event, 'alphaBox', syncAlphaButton)
                    "
                    @touchstart="
                        onclick($event, clickAlpha) &&
                            moveable($event, 'alphaBox', syncAlphaButton)
    ...
    methods: {
        ...
        moveable(evt, boxName, callback) {
            ...
            let leaveMove = () => {
                ...
                window.removeEventListener("touchend", leaveMove);
                setTimeout(() => {
            ...
            window.addEventListener("touchend", leaveMove);
        },
        onclick(evt, callback) {
            if (!this.clickLock) {
                callback(
                    evt.offsetX / evt.target.clientWidth,
                    evt.offsetY / evt.target.clientHeight
                );
                return true;
            }
            return false;
