...
<div :class="'fv-'+$theme+'-DetailsList'">
    <div class="fv-details-list-head" :class="{'fv-custom-head': true}" ref="list_head">
        ...
        <span v-show="item.show && valueTrigger(item.visible)" v-for="(item, index) in thisHead" class="col" :key="`head: ${index}`" :style="{'min-width': colWidth[index], width: colWidth[index], background: styles.listHead.background}">
            ...
            <spliter style="height: 100%;" @mousedown="resizeDown($event, index)" @mousemove="resizeMove($event, index)" @touchstart="resizeDown($event.targetTouches[0], index)" @touchmove="resizeMove($event.targetTouches[0], index)"></spliter>
        </span>
    </div>
        ...
        <div v-for="(gi, i) in thisGroup" :key="`group: ${i}`">
            <div class="content-row" :class="{choose: isGroupChooseAll(gi), 'fv-custom-group-row': true}">
                <slot name="group" :item="gi" :index="i" :isMulti="multiSelection" :isChoose="isGroupChooseAll(gi)">
                    ...
                    <slot name="group_content" :item="gi" :index="i" :isMulti="multiSelection" :isChoose="isGroupChooseAll(gi)">
...
export default {
    ...
    beforeDestroy () {
        ...
        this.$RevealMasked.destroy(this.FR_Table_Head);
    }
};
</script>
